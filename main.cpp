...
struct CacheEntry {
    ...
    static std::optional<CacheEntry> from_json(const json& v) {
        if (!v.is_object()
            || !v.contains("account") || !v["account"].is_string()
            || !v.contains("password") || !v["password"].is_string()
            || !v.contains("device_id") || !v["device_id"].is_string()
            || !v.contains("open_id") || !v["open_id"].is_string()
            || !v.contains("combo_token_raw") || !v["combo_token_raw"].is_string()
            || !v.contains("cookies"))
            return std::nullopt;

        CacheEntry e;
        e.account             = v.value("account", "");
        e.password            = v.value("password", "");
        e.device_id           = v.value("device_id", "");
        e.seed_id             = v.value("seed_id", "");
        e.seed_time           = v.value("seed_time", "");
        e.device_fp           = v.value("device_fp", "");
        e.lifecycle_id        = v.value("lifecycle_id", "");
        e.webapp_lifecycle_id = v.value("webapp_lifecycle_id", "");
        e.open_id             = v.value("open_id", "");
        e.combo_token_raw     = v.value("combo_token_raw", "");
        e.saved_at            = v.value("saved_at", 0LL);

        e.ext_device_memory        = v.value("ext_device_memory", "");
        e.ext_hardware_concurrency = v.value("ext_hardware_concurrency", "");
        e.ext_canvas               = v.value("ext_canvas", "");
        e.ext_webgl                = v.value("ext_webgl", "");
        ...
        if (cobj.is_object()) {
            for (auto& [name, ci] : cobj.items()) {
                ...
                CookieInfo info;
                info.value  = ci.value("value", "");
                info.domain = ci.value("domain", "");
                info.path   = ci.value("path", "");
...
public:
    ...
    explicit CacheRepository(std::string dir = "")
        : _dir(dir.empty() ? default_dir() : fs::path(std::move(dir))) {
        std::error_code ec;
        fs::create_directories(_dir, ec);
        if (ec)
            LOG_WARN("Failed to create cache directory {}: {}", _dir.string(), ec.message());
    }

    std::optional<CacheEntry> load_for_account(const std::string& account) const {
        auto path = path_for_account(account);
        auto e = load_path(path);
        if (e && e->account != account) {
            LOG_WARN("Cache account mismatch in {}, ignoring.", path.string());
            return std::nullopt;
        }
        return e;
    }

    std::vector<CacheEntry> load_all() const {
        std::vector<CacheEntry> entries;
        std::error_code ec;
        if (!fs::exists(_dir, ec)) return entries;
        ...
        for (const auto& item : fs::directory_iterator(_dir, ec)) {
            ...
            if (path.extension() != ".json" || name.rfind("session_", 0) != 0)
                continue;

            auto e = load_path(path);
            if (!e) continue;

            if (path != path_for_account(e->account)) {
                LOG_WARN("Cache filename/account mismatch: {}, ignoring.", path.string());
                continue;
            }
    ...
    void save(const CacheEntry& entry) const {
        if (entry.account.empty())
            throw std::runtime_error("Cannot save cache without account");

        auto path = path_for_account(entry.account);
        std::ofstream f(path, std::ios::trunc);
        if (!f.is_open()) throw std::runtime_error("Cannot write cache: " + path.string());
        ...
        fs::permissions(path,
                        ...
                        ec);
        if (ec)
            LOG_WARN("Failed to restrict cache permissions {}: {}", path.string(), ec.message());
#endif
        LOG_DEBUG("Session cache saved to {}", path.string());
    }

    void remove_for_account(const std::string& account) const {
        std::error_code ec;
        auto path = path_for_account(account);
        if (fs::remove(path, ec))
            LOG_INFO("Session cache deleted for account {}.", account);
        else if (ec)
            LOG_WARN("Failed to delete cache {}: {}", path.string(), ec.message());
    }

    fs::path path_for_account(const std::string& account) const {
        return _dir / ("session_" + account_hash(account) + ".json");
    }

private:
    fs::path _dir;

    static std::string account_hash(const std::string& account) {
        unsigned char digest[EVP_MAX_MD_SIZE];
        unsigned int dlen = 0;
        if (EVP_Digest(account.data(), account.size(), digest, &dlen,
                       EVP_sha256(), nullptr) != 1)
            throw std::runtime_error("Failed to hash account for cache filename");
    ...
    std::optional<CacheEntry> load_path(const fs::path& path) const {
        std::ifstream f(path);
        ...
        try {
            ...
            if (!e)
                LOG_WARN("Cache file {} is malformed or uses the old format, ignoring.", path.string());
            return e;
        } catch (const std::exception& ex) {
            LOG_WARN("Failed to parse cache file {}: {}", path.string(), ex.what());
            return std::nullopt;
        }
    }
};
...
class CheckInFacade {
public:
    CheckInFacade(std::string account,
                  std::string password,
    ...
    bool run(std::optional<CacheEntry> cache) {
        std::string app_version = util::fetch_cloudgame_version();

        if (cache && cache->account == _account && cache->password != _password) {
            LOG_INFO("Password changed for account {}, ignoring old cache and doing a fresh login.", _account);
            cache.reset();
        } else if (cache && cache->account != _account) {
            LOG_WARN("Loaded cache belongs to another account, ignoring it.");
            cache.reset();
        }
        ...
        if (cache) {
            ...
            LOG_INFO("Account {} | found cached session (saved at {}), trying fast path.",
                     _account, std::string(buf));

            result = try_cached(session, *cache);
            if (!result) {
                LOG_INFO("Account {} | cached session expired, re-authenticating with pinned device identity.",
                         _account);
                session = HttpSession{};
                result  = try_full(session, &*cache, app_version);
            }
        } else {
            LOG_INFO("Account {} | no usable cache, running full login flow.", _account);
            result = try_full(session, nullptr, app_version);
        }

        if (!result) {
            LOG_ERROR("Account {} | all login attempts failed.", _account);
            return false;
        }
...
static void print_usage(const char* exe) {
    std::cerr
        << "usage:\n"
        << "  " << exe << "                  # login/check-in all cached accounts\n"
        << "  " << exe << " account password # login/check-in only this account\n";
}
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (argc == 1) {
            auto caches = repo.load_all();
            if (caches.empty()) {
                LOG_WARN("No account cache files found. Run once with: {} account password", argv[0]);
                return 1;
            }

            LOG_INFO("Found {} cached account(s); processing all.", caches.size());
            int failed = 0;
            for (const auto& cache : caches) {
                LOG_INFO("========== account {} ==========" , cache.account);
                try {
                    if (!CheckInFacade(cache.account, cache.password, repo).run(cache))
                        ++failed;
                } catch (const std::exception& ex) {
                    ++failed;
                    LOG_ERROR("Account {} | unhandled exception: {}", cache.account, ex.what());
                }
            }

            if (failed != 0) {
                LOG_ERROR("Completed with {} failed account(s).", failed);
                return 1;
            }
            return 0;
        }
        ...
        if (argc == 3) {
            std::string account  = argv[1];
            std::string password = argv[2];

            auto cache = repo.load_for_account(account);
            return CheckInFacade(account, password, repo).run(std::move(cache)) ? 0 : 1;
        }
